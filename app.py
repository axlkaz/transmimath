...
def option(opt):
    ...
    if opt == 1:
                        ...
                        for j, val in enumerate(current_matrix[c]):
                            if val and j not in contagiados:
                                if random.random()*100 <= tasa:
                                    new.add(j)
                if not new: break
                for n in new: dias_cont[n] = dias
                contagiados |= new
                historial.append([x+1 for x in new])
            simulations.append({'emp0': emp0, 'dias': dias, 'total': len(contagiados), 'hist': historial})
            current_contagions = set(contagiados)
            return render_template('result1.html', dias=dias, total=len(contagiados), N=N, Y=Y)
        ...
        if request.method == 'POST':
            ...
            # Convert path nodes to 1-based indices for template
            path_display = [x+1 for x in path]
            return render_template('result3.html', path=path_display, dist=dist, e1=e1+1, e2=e2+1)
        return render_template('option3.html')
    elif opt == 4:
        if request.method == 'POST':
            t = int(request.form['t']) - 1
            status = t in current_contagions
            return render_template('result4.html', t=t+1, status=status)
